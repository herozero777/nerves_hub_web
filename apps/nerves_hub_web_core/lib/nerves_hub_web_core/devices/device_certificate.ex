defmodule NervesHubWebCore.Devices.DeviceCertificate do
  ...
  defp maybe_add_fingerprints(changeset) do
    case {get_field(changeset, :der), needs_fingerprints?(changeset)} do
  ...
  defp needs_fingerprints?(changeset) do
    fp = get_field(changeset, :fingerprint)
    pkfp = get_field(changeset, :public_key_fingerprint)

    is_nil(fp) or is_nil(pkfp)
  end
